...
{
    class DamageWorker_Lightning : DamageWorker
    {
        public override DamageWorker.DamageResult Apply(DamageInfo dinfo, Thing thing)
        {
            ...
            Hediff hediff = HediffMaker.MakeHediff(RWBYDefOf.RWBY_Lightning_Damage, pawn);
            pawn.health.AddHediff(hediff);
            return this.ApplyToPawn(dinfo, pawn);
        }

        // Base Game Code Copy&Paste to avoid Harmony Patch

        #region "Base Game Code Copy&Paste to avoid Harmony Patch"
        ...
        private DamageWorker.DamageResult ApplyToPawn(DamageInfo dinfo, Pawn pawn)
        {
            ...
            {
                DamageWorker_Lightning.PlayWoundedVoiceSound(dinfo, pawn);
        ...
        private void ApplyDamageToPart(DamageInfo dinfo, Pawn pawn, DamageWorker.DamageResult result)
        {
            ...
            if (DamageWorker_Lightning.IsHeadshot(dinfo, pawn))
            {
