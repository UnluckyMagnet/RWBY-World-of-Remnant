...
{
    class Verb_AbilitySummonArmaGigasSword : Verb_UseAbilitySemblanceBase
    {
        protected override bool TryCastShot()
        {
            ...
            else
            {
                for (int i = 0; i < this.TargetsAoE.Count; i++)
                {
                    ...
                    glyph.TryGetComp<CompGlyphSummon>().SetData(GenTicks.SecondsToTicks(1.5f), RWBYDefOf.Grimm_ArmaGigasSword_Summoned);
                    ...
                    CasterPawn.TryGetComp<CompAbilityUserAura>().AbilityData.AllPowers.Find(a => a.Def == RWBYDefOf.Weiss_SummonLancer).CooldownTicksLeft = 3600;
